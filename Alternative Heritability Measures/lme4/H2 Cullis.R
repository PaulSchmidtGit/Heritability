...
vc   <- as.data.table(VarCorr(mod)) # extract estimated variance components (vc)

# R = varcov-matrix for error term
n    <- length(summary(mod)$residuals) # numer of observations
...
# Design Matrices
X <- as.matrix(getME(mod, "X")) # Design matrix fixed effects
Z <- as.matrix(getME(mod, "Z")) # Design matrix random effects
